"""
Message Classifier using OpenAI API
Ported from slackAgent.ts
"""

import os
import json
from openai import OpenAI
from schema import ClassificationV1

# Initialize OpenAI client
client = OpenAI(
    api_key=os.getenv('OPENAI_API_KEY'),
    timeout=20.0,  # 20 second timeout (from openaiClient.ts)
    max_retries=0   # No automatic retries (handled externally)
)

# Classification instructions (copied from slackAgent.ts)
...
def classify_message(message: str, message_timestamp: str = None) -> ClassificationV1:
    ...
    Returns:
        ClassificationV1: Parsed classification result
    ...
    try:
        classification = ClassificationV1.model_validate_json(result_text)
        classification.validate_keys()  # Additional validation
...
if __name__ == "__main__":
    ...
    async def test_classifier():
        test_message = "We got an offer on 123 Main St! Need to schedule closing by Friday."
        timestamp = datetime.utcnow().isoformat()

        print("Classifying message:", test_message)
        result = classify_message(test_message, timestamp)
